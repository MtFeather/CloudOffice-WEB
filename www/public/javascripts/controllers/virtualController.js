function deletevm(eid){
	this.confirmVm('刪除成員','您確定要刪除此成員嗎?','刪除');
...
function allset(oid){
	if($('#btn'+oid).val()=='start'){
		this.confirmVm('全部開機','您確定要開啟此會議所有硬碟嗎？','確定');
	        $('.modal-confirm .submit').click(function(){ startvm(oid); });
	} else if($('#btn'+oid).val()=='close'){
		this.confirmVm('全部關機','您確定要關閉此會議所有硬碟嗎？','確定');
...
this.successdel = function()
{
        ...
        $('.modal-alert .modal-body p').html('成員已刪除成功');
...
this.successred = function()
{
        ...
        $('.modal-alert .modal-body p').html('此成員已還原成功');
        $('.modal-alert #ok').click(function(){window.location.reload();});
	$('.modal-alert').modal('show');
}
...
this.successsyncori = function()
{
        ...
        $('.modal-alert .modal-body p').html('正在同步硬碟，請至<a href="department_show">環境資訊</a>查看');
        $('.modal-alert #ok').click(function(){window.location.reload();});
        $('.modal-alert').modal('show');
}
